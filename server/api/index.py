...
def safe_float(val, default=0.0):
    """Safely convert value to float, defaulting if it fails."""
    try:
        if val is None:
            return default
        return float(val)
...
def load_and_prepare_data():
    ...
    try:
        ...
        print(f"❌ [ERROR] Không tìm thấy file {DATA_FILE} tại {DATA_FILE}.")
    except Exception as e:
        print(f"❌ [ERROR] {e}")


def load_users_csv():
    """Pre-load users.csv into user_ratings / user_enrollments."""
...
if __name__ == "__main__":
    ...
    print("   Content-based : GET  /api/recommend?course_name=<name>")
    print("   User CF       : GET  /api/recommend/user?user_id=<id>")
    print("   Enroll        : POST /api/enroll  {user_id, course_idx}")
    print("   Rate          : POST /api/rate    {user_id, course_idx, rating}")
    print("   Profile       : GET  /api/profile?user_id=<id>")
